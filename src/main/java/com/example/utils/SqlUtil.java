...
public class SqlUtil {
  ...
  public List<FinalData> sqlAllUtil() {
    class tool {
       ...
       * @return 最终数值字符串
       */
      String executeSQL(FinalData finalData,
    ...
    for (FundNetVal fundNetVal : fundNetValsList) {
      ...
      finalData.setNearlyOneMonth(
          new tool().getUnitNetVal(finalData, "1 MONTH"));
      // 近三月
      finalData.setNearlyThreeMonth(
          new tool().getUnitNetVal(finalData, "3 MONTH"));
      // 近一年
      ...
      finalData.setNearlyThreeYear(
          new tool().getUnitNetVal(finalData, "3 YEAR"));
      // 今年来
      finalData.setForNearlyOneYear(
          new tool().getForNearlyOneYearUnitNetVal(finalData));
      // 成立以来
      finalData.setSinceItsEstablishment(
          new tool().getSinceItsEstablishment(finalData));
